...
export function Pots() {
  ...
  const handleAddMoney = async (potId, amount) => {
    ...
    try {
      await addFundsToGoal(potId, {
        ...
        description: `Added ${formatCurrency(numericAmount)} to savings goal`,
      })
      setShowAddMoney(null)
      setCustomAmount("")
    } catch (error) {
  ...
  const handleWithdrawMoney = async (potId, amount) => {
    try {
      await withdrawFromGoal(potId, {
        amount: amount,
        description: `Withdrew ${formatCurrency(amount)} from savings goal`,
      })
      setShowWithdrawMoney(null)
      setCustomAmount("")
    } catch (error) {
  ...
  const handleAllocateFromSavings = async (potId, amount) => {
    try {
      await allocateToGoal({
        amount: amount,
        goal_id: potId,
        description: `Allocated ${formatCurrency(amount)} from savings account`,
      })
      setShowAllocateFromSavings(null)
      setCustomAmount("")
    } catch (error) {
  ...
  const handleQuickAmount = (potId, amount) => {
    handleAddMoney(potId, amount)
  }
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-blue-200 text-xs mt-1">
                💡 Every time you earn income, {savingsAccount.auto_save_percentage}% is automatically added to your
                savings!
              </p>
            </div>
            <div className="text-right">
              ...
              <div key={pot.id} className="bg-white p-6 rounded-xl shadow-sm border hover:shadow-md transition-shadow">
                ...
                <div className="space-y-4">
                  ...
                  {showAddMoney === pot.id ? (
                    ...
                    /* Enhanced Action Buttons with more predefined amounts */
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        {predefinedAmounts.slice(0, 4).map((amount) => (
                          <button
                            key={amount.value}
                            onClick={() => handleQuickAmount(pot.id, amount.value)}
                            className="bg-blue-600 text-white py-2 px-3 rounded-md text-sm hover:bg-blue-700 transition-colors"
                          >
                            Add {amount.label}
                          </button>
