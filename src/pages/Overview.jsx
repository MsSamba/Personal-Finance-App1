...
export function Overview() {
  const { transactions, transactionSummary, transactionsLoading, balance, savingsAccount, savingsGoals, budgets } =
    useFinance()
  ...
  const calculateAvailableBalance = () => {
    ...
    const allocatedBudgets = Array.isArray(budgets)
      ? budgets.reduce((total, budget) => total + (budget.limit || 0), 0)
      : 0
    return currentBalance - allocatedBudgets
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-gray-900 text-white p-6 rounded-xl">
          <h3 className="text-sm font-medium text-gray-300">Current Balance</h3>
          <p className="text-2xl font-bold mt-2">{formatCurrency(balance ?? 0)}</p>
          <p className="text-xs text-gray-400 mt-1">Calculated from {transactions?.length || 0} transactions</p>
        </div>
        ...
        <div className="bg-blue-600 text-white p-6 rounded-xl">
          ...
          <p className="text-xs text-blue-200 mt-1">After budget allocations</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border">
          ...
          <p className="text-xs text-gray-500 mt-1">
            From {transactionSummary?.expense_count || 0} expense transactions
          </p>
        </div>
        ...
        {savingsAccount && (
          <div className="bg-white p-6 rounded-xl shadow-sm border">
            <h3 className="text-sm font-medium text-gray-600">Savings Balance</h3>
            <p className="text-2xl font-bold text-purple-600 mt-2">{formatCurrency(savingsAccount.balance || 0)}</p>
            <p className="text-xs text-gray-500 mt-1">{savingsGoals.length} active goals</p>
          </div>
        )}
      </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {transaction.type === "expense" ? transaction.category || "Expense" : "Income"}
                    </p>
                  </div>
                </div>
                <div className="text-right">
